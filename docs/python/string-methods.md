...
This is most useful when applying one of these methods to many strings at once in say, a **[`map`](http://web.archive.org/web/20170405123520/http://stackoverflow.com/documentation/python/333/map-function)**  function.
...
While the official python documentation is, as usual, thorough enough, [pyformat.info](http://web.archive.org/web/20170405123520/http://pyformat.info/) has a great set of examples with detailed explanations.
...
See [String Formatting](http://web.archive.org/web/20170405123520/http://stackoverflow.com/documentation/python/1019/string-formatting) for additional information. `str.format()` was proposed in [PEP 3101](http://web.archive.org/web/20170405123520/https://www.python.org/dev/peps/pep-3101/).
...
Python's `str` type also has a method for replacing occurences of one sub-string with another sub-string in a given string. For more demanding cases, one can use [`re.sub`](http://web.archive.org/web/20170405123520/http://stackoverflow.com/documentation/python/632/regular-expressions/2068/replacing-with-a-string#t=201607231554255817393).
...
`reversed()` can be wrapped in a call to [`''.join()` to make a string](http://web.archive.org/web/20170405123520/http://stackoverflow.com/documentation/python/278/string-methods/10177/join-a-list-of-strings-into-one-string) from the iterator.
...
While using `reversed()` might be more readable to uninitiated Python users,  using extended [slicing](http://web.archive.org/web/20170405123520/https://stackoverflow.com/documentation/python/289/indexing-and-slicing/1042/basic-slicing) with a step of `-1` is faster and more concise. Here , try to implement it as function:
...
But the shortest way to test if a string is empty or just contains whitespace characters is to use [`strip`](http://web.archive.org/web/20170405123520/http://stackoverflow.com/documentation/python/278/string-methods/8777/stripping-unwanted-leading-trailing-characters-from-a-string#t=201608150741303855198)(with no arguments it removes all leading and trailing whitespace characters)
