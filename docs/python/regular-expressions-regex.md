...
A "raw string literal" is a slightly different syntax for a string literal, in which a backslash, `\`, is taken as meaning "just a backslash" (except when it comes right before a quote that would otherwise terminate the literal) -- no "escape sequences" to represent newlines (`\n`), tabs (`\t`), backspaces (`\`), form-feeds (`\r`), and so on. In normal string literals, each backslash must be doubled up to avoid being taken as the start of an escape sequence.

Hence, `r"\n"` is a string of 2 characters `\` and `n`. The regex patterns also uses backslash, eg. `\d` refer to any digit character. We avoid having to double escape our strings (`"\\d"`) by using raw strings (`r"\d"`).
...
Compiling a pattern allows it to be reused later on in a program. However, note that Python caches recently-used expressions ([docs](http://web.archive.org/web/20170405062625/https://docs.python.org/3/library/re.html#re.compile), [SO answer](http://web.archive.org/web/20170405062625/http://stackoverflow.com/a/452143/1240268)), so **"programs that use only a few regular expressions at a time needn’t worry about compiling regular expressions"**.
...
For the complete list of all available flags check the [docs](http://web.archive.org/web/20170405062625/https://docs.python.org/2/library/re.html#module-contents)

### **Inline flags**

From the [docs](http://web.archive.org/web/20170405062625/https://docs.python.org/2/library/re.html#regular-expression-syntax):
...
Partial credit : [http://stackoverflow.com/a/1325265/2697955](http://web.archive.org/web/20170405062625/http://stackoverflow.com/a/1325265/2697955)
...
From the [docs](http://web.archive.org/web/20170405062625/https://docs.python.org/2/library/re.html#re.MatchObject.group):
...
Here the "apple" occurs twice which can be solved with so called **backtracking control verbs** which are supported by the newer [**`regex`**](http://web.archive.org/web/20170405062625/https://pypi.python.org/pypi/regex) module. The idea is:
...
Replacements can be made on strings using [`re.sub`](http://web.archive.org/web/20170405062625/https://docs.python.org/2/library/re.html#re.sub).
...
However, if you make a group ID like '10', [this doesn't work](http://web.archive.org/web/20170405062625/https://docs.python.org/2/library/re.html#re.sub): `\10` is read as 'ID number 1 followed by 0'. So you have to be more specific and use the `\g&lt;i&gt;` notation:
